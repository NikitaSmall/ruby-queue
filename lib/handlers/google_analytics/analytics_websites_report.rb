module Handlers
  # Этого класса не должно быть в проекте. Он должен быть разнесен по нескольким акторам, которые умеют
  # 1. формировать запрос для получения profile
  # 2. формировать запрос для получения webproperties
  # 3. выполнять подготовленный API запрос (актор, которому передавать результат, передается в качестве аргумента)
  # 4. Разбирать ответ от актора №3 и готовить хеши для сохранения в базу
  # 5. Сохранять объект в базу
